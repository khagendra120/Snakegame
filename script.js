...
const SPEED_STEP = 0.25;         // Speed increases per fruit
...
let gameOver = false;
let score = 0;
let highScore = 0; // Store in memory instead of localStorage
...
function reset(){
  ...
  score = 0; speed = SPEED_BASE; acc = 0; lastTick = 0;
...
function placeFood(){
  ...
  food = empty[Math.floor(Math.random()*empty.length)] || null;
}
...
function start(){ 
  ...
  paused = false; 
  hideOverlay();
  lastTick = performance.now(); // Reset timing
}
...
function togglePause(){ 
  ...
  if(paused) {
    ...
    hideOverlay();
    lastTick = performance.now(); // Reset timing on resume
  }
}
...
function showGameOver(){ 
  ...
  stateSubtitle.textContent='Press R to restart or arrow key to start'; 
  showOverlay(); 
}
...
function step(dt){
  ...
  acc -= msPerMove; // handle catch-up if lag


  // Apply buffered direction (prevents 180° turn bug)
  ...
  if(food && head.x === food.x && head.y === food.y){
    ...
    speed = SPEED_BASE + (score/10)*SPEED_STEP; // increase speed gradually
...
function setDirection(x,y){
  if(!running || paused || gameOver) return;
  if((x===-dir.x && y===0) || (y===-dir.y && x===0)) return; // prevent direct reverse
  nextDir = {x,y};
}
...
window.addEventListener('keydown', e=>{
  ...
  // When game over, restart on any arrow key press and set direction
  if(gameOver) {
  ...
  // Start game on first Up arrow key press if not running and not game over
  if (!running && !gameOver && (k === 'arrowup' || k === 'w')) {
    e.preventDefault();
    start();
    setDirection(0, -1); // Set initial direction upward
    return;
  }
